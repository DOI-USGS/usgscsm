...
std::string UsgsAstroFramePlugin::getStateFromISD(csm::Isd imageSupportData) const {
    ...
    state["m_model_name"] = imageSupportData.param("model_name");
    state["m_startingDetectorSample"] = atof(imageSupportData.param("starting_detector_sample").c_str());
    state["m_startingDetectorLine"] = atof(imageSupportData.param("starting_detector_line").c_str());
    ...
    if (imageSupportData.param("sensor_location") == "") {
      ...
      state["m_currentParameterValue"] = {0,0,0,0,0,0,0};
      ...
      if (unit == json("")) {
        ...
        state["m_currentParameterValue"][0] = metric_conversion(state["m_currentParameterValue"][0], unit);
        state["m_currentParameterValue"][1] = metric_conversion(state["m_currentParameterValue"][1], unit);
        state["m_currentParameterValue"][2] = metric_conversion(state["m_currentParameterValue"][2], unit);
      }
    }
    ...
    if (imageSupportData.param("sensor_velocity") == "") {
      ...
      state["m_spacecraftVelocity"] = {0,0,0};
    ...
    state["m_currentParameterValue"][3] = atof(imageSupportData.param("sensor_orientation", 0).c_str());
    state["m_currentParameterValue"][4] = atof(imageSupportData.param("sensor_orientation", 1).c_str());
    state["m_currentParameterValue"][5] = atof(imageSupportData.param("sensor_orientation", 2).c_str());
    state["m_currentParameterValue"][6] = atof(imageSupportData.param("sensor_orientation", 3).c_str());
    ...
    state["m_ephemerisTime"] = atof(imageSupportData.param("center_ephemeris_time").c_str());
    if (imageSupportData.param("center_ephemeris_time") == "") {
      missingKeywords.push_back("center_ephemeris_time");
    }

    state["m_nLines"] = atoi(imageSupportData.param("image_lines").c_str());
    state["m_nSamples"] = atoi(imageSupportData.param("image_samples").c_str());
    ...
    state["m_iTransL"] = {0,0,0};
    state["m_iTransL"][0] = atof(imageSupportData.param("focal2pixel_lines", 0).c_str());
    state["m_iTransL"][1] = atof(imageSupportData.param("focal2pixel_lines", 1).c_str());
    state["m_iTransL"][2] = atof(imageSupportData.param("focal2pixel_lines", 2).c_str());
    ...
    state["m_iTransS"] = {0,0,0};
    state["m_iTransS"][0] = atof(imageSupportData.param("focal2pixel_samples", 0).c_str());
    state["m_iTransS"][1] = atof(imageSupportData.param("focal2pixel_samples", 1).c_str());
    state["m_iTransS"][2] = atof(imageSupportData.param("focal2pixel_samples", 2).c_str());
    ...
    double determinant = state["m_iTransL"][1].get<float>() * state["m_iTransS"][2].get<float>() -
                         state["m_iTransL"][2].get<float>() * state["m_iTransS"][1].get<float>();

    state["m_transX"] = {0,0,0};
    state["m_transX"][1] =   state["m_iTransL"].at(1).get<float>() / determinant;
    state["m_transX"][2] = - state["m_iTransS"].at(1).get<float>() / determinant;
    state["m_transX"][0] = - (state["m_transX"].at(1).get<float>() * state["m_iTransL"].at(0).get<float>() +
                               state["m_transX"].at(2).get<float>() * state["m_iTransS"].at(0).get<float>());

    state["m_transY"] = {0,0,0};
    state["m_transY"][1]= - state["m_iTransL"].at(2).get<float>() / determinant;
    state["m_transY"][2] =   state["m_iTransS"].at(2).get<float>() / determinant;
    state["m_transY"][0] = - (state["m_transY"].at(1).get<float>() * state["m_iTransL"].at(0).get<float>() +
                               state["m_transY"].at(2).get<float>() * state["m_iTransS"].at(0).get<float>());
    ...
    if (imageSupportData.param("radii") == "") {
      ...
      if (unit == json("")) {
        ...
        unit = unit.get<std::string>();
        state["m_minorAxis"] = metric_conversion(state["m_minorAxis"], unit);
        state["m_majorAxis"] = metric_conversion(state["m_majorAxis"], unit);
      }
    }
    ...
    if (imageSupportData.param("reference_height") == "") {
      ...
      if (unit == json("")) {
        ...
        unit = unit.get<std::string>();
        state["m_minElevation"] = metric_conversion(state["m_minElevation"], unit);
        state["m_maxElevation"] = metric_conversion(state["m_maxElevation"], unit);
      }
    }
...
csm::Model *UsgsAstroFramePlugin::constructModelFromState(const std::string& modelState,
                                                csm::WarningList *warnings) const {
    UsgsAstroFrameSensorModel *sensor_model = 0;
    ...
    // mdsensor_model->m_boresight = state["m_boresight"].get<std::vector<double>>();
    mdsensor_model->m_iTransL = state["m_iTransL"].get<std::vector<double>>();
    mdsensor_model->m_iTransS = state["m_iTransS"].get<std::vector<double>>();
    mdsensor_model->m_transX = state["m_transX"].get<std::vector<double>>();
    mdsensor_model->m_transY = state["m_transY"].get<std::vector<double>>();
    mdsensor_model->m_ccdCenter = state["m_ccdCenter"].get<std::vector<double>>();
    mdsensor_model->m_spacecraftVelocity = state["m_spacecraftVelocity"].get<std::vector<double>>();
    mdsensor_model->m_sunPosition = state["m_sunPosition"].get<std::vector<double>>();
...
csm::Model *UsgsAstroFramePlugin::constructModelFromISD(const csm::Isd &imageSupportData,
  ...
  std::string state = getStateFromISD(imageSupportData);
  std::string model_name = imageSupportData.param("model_name");

  sensorModel = (UsgsAstroFrameSensorModel*)constructModelFromState(state);
  return sensorModel;
}
...
std::string UsgsAstroFramePlugin::getModelNameFromModelState(const std::string &modelState,
  ...
  std::string name = state.value<std::string>("m_model_name", "");
