...
TEST_F(SarSensorModel, adjustedPositionVelocity) {
  ...
  EXPECT_NEAR(adjPosition.x, 4737399.9991294323, 1e-2);
  EXPECT_NEAR(adjPosition.y, 0.2, 1e-2);
  EXPECT_NEAR(adjPosition.z, -18.0667951486008143, 1e-2);
  EXPECT_NEAR(adjVelocity.x, -28.0667951486008143, 1e-8);
  EXPECT_NEAR(adjVelocity.y, 0.5, 1e-8);
  EXPECT_NEAR(adjVelocity.z, -1737399.9991294323, 1e-2);
}
