...
TEST_F(ConstVelocityProjectedSensorModel, Center) {
  ...
  EXPECT_NEAR(groundPt.x, 9.8989858043564229, 1e-10);
  EXPECT_NEAR(groundPt.y, -1.4118584049198812, 1e-10);
  EXPECT_NEAR(groundPt.z, 0.12936726636640034, 1e-10);
}
...
TEST_F(ConstVelocityProjectedSensorModel, OffBody) {
  ...
  csm::EcefCoord groundPt = sensorModel->imageToGround(imagePt, 0.0);
  EXPECT_NEAR(groundPt.x, 9.8988664296556497, 1e-10);
  EXPECT_NEAR(groundPt.y, -1.4125636827694117, 1e-10);
  EXPECT_NEAR(groundPt.z, 0.1307946862733107, 1e-10);
}
...
TEST_F(ConstVelocityProjectedSensorModel, RemoteImageLocus) {
  ...
  EXPECT_NEAR(locus.point.z, 1.1193790655763731, 1e-10);
  EXPECT_LT(precision, 0.001);
  EXPECT_TRUE(warnings.empty());
}